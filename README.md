...
##### - Об'єднання операцій обслуговування потягів (патерн Фасад)
> **Операції обслуговування:**
>> - Заправка потяга
>> - Технічне обслуговування
>> - Очищення вагонів

##### - Захист доступу до управління поїздом (патерн Замісник)
> **Типи доступу:**
>> - Повний доступ (адміністратор)
>> - Обмежений доступ (користувач)

##### - Розподіл способів обробки даних потягів і типів інтерфейсів (патерн Міст)
> **Способи обробки:**
>> - Реальна обробка даних
>> - Імітаційна обробка даних
...
1. **Об'єднання операцій обслуговування потягів (Фасад)**
    - **Завдання:**
        - Створити класи для кожної операції обслуговування: `RefuelService`, `MaintenanceService`, `CleaningService`.
        - Створити клас `TrainServiceFacade`, який об'єднує ці операції та надає простий інтерфейс для їх використання.

2. **Захист доступу до управління поїздом (Замісник)**
    - **Завдання:**
        - Створити інтерфейс `TrainController` з методами управління поїздом.
        - Реалізувати класи `RealTrainController` та `ProxyTrainController` для контролю доступу до управління поїздом в залежності від ролі користувача.

3. **Розподіл способів обробки даних потягів і типів інтерфейсів (Міст)**
    - **Завдання:**
        - Створити інтерфейси для обробки даних: `TrainDataProcessor` і класи для реальної та імітаційної обробки.
        - Створити інтерфейси для типів інтерфейсів: `TrainInterface` і класи для графічного та консольного інтерфейсів.
        - Використати патерн Міст для з'єднання обробки даних та типів інтерфейсів, що дозволяє змінювати їх незалежно один від одного.

#### Пояснення:

1. **Фасад (Facade)**: Патерн використовується для об'єднання операцій обслуговування потягів. Клас `TrainServiceFacade` надає простий інтерфейс для виконання заправки, технічного обслуговування та очищення вагонів, приховуючи складність цих операцій.

2. **Замісник (Proxy)**: Патерн використовується для захисту доступу до управління поїздом. Клас `ProxyTrainController` контролює доступ до реального контролера `RealTrainController`, перевіряючи права користувача перед виконанням операцій відправки та прибуття.

3. **Міст (Bridge)**: Патерн використовується для розподілу способів обробки даних потягів і типів інтерфейсів. Абстрактний клас `TrainInterface` містить посилання на об'єкт `TrainDataProcessor`, що дозволяє реалізовувати різні комбінації способів обробки даних (реальна або імітаційна) та типів інтерфейсів (графічний або консольний).
